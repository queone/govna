...
A user runs `govna <subcommand>`. `version`/`ver`/`--version`, `render-canon`, `drift-scan`, `apply`, and `rm` produce real output; an unrecognized subcommand prints usage to stderr and exits 2. `apply` bootstraps the current directory: writes the full canon set (hunk-merging hybrid files rather than overwriting them when they already exist), a `CLAUDE.md` symlink, and a `govna/ac<N>-govna-apply.md` adoption record; `--init-git` optionally initializes git. When a `governa/` directory is detected, `apply` also emits `govna/ac<N>-govna-migrate-from-governa-<version>.md` tracking its review and removal. `rm` plans (but does not perform) canon removal: it emits `govna/ac<N>-govna-rm-<version>.md`, with on-demand comparison commands in place of pre-computed diffs, for Director review.
