...
- Never overwrite independent utility versions automatically.
...
- Limit `plan.md` structure to its existing top-level sections (`## Product Direction`, `## Ideas To Explore`).
- Add new plan content only as IE bullets.
...
Note: when mirroring a change to `AGENTS.md` or a doc it references into `templates/base/AGENTS.md` or `templates/overlays/*/files/`, exclude any content that's only meaningful inside govna's own source checkout (e.g., references to `templates/`, govna's own build/release process) — a consumer repo has no `templates/` tree to act on. This section's own bullets are the clearest example: only the last one ("Follow existing repo patterns...") ships in the shipped templates' `## Project Rules` seed. Everything else here — the two `Mirror every...` bullets, every utility-versioning/release-mechanics rule, `plan.md`/IE-tracking conventions, and "Keep generated repos self-contained" — describes govna's own project specifically (a multi-utility-versioning, IE-tracking, AC-driven template tool) and is deliberately absent from what a fresh consumer gets seeded with. `## Project Rules` is consumer-owned from the start, not pre-filled with govna's own rules as if they were a neutral example.
