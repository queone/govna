...
- IE10: Once govna's port is complete and it's ready to stand as the canonical implementation rather than a dependent of governa (Director determines readiness — do not self-trigger on an empty Ideas To Explore list alone), rescind "ported from governa" / "Rust port of Go governa" framing across the repo: `README.md`'s tagline, `arch.md`'s `## Purpose` section, this file's own Product Direction paragraph, `govna/*.md` docs' provenance prose (`operator-contract-rationale.md`, `drift-scan.md`, `canon-cycle.md`, etc.), `AGENTS.md`'s governa-comparison language, and `src/*.rs` module-doc-comments citing governa source packages. Explicitly excludes `CHANGELOG.md` (historical record, never rewritten) and git commit messages (immutable history).
- IE12: Prohibit bare `cargo` compilation commands outside `./build.sh` (and settle whether a `.cargo/config.toml` guardrail is warranted) to stop stray `./target/` pollution in the repo root → govna/ac11-bare-cargo-target-guard.md
