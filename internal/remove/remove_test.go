...
func TestRemovalFreshAndIdempotent(t *testing.T) {
	...
	if stdout.String() != "wrote govna/ac1-govna-rm-v0.31.0.md\n" {
		t.Fatalf("stdout=%q", stdout.String())
	}
	path := filepath.Join(root, "govna", "ac1-govna-rm-v0.31.0.md")
	...
	stdout.Reset()
	stderr.Reset()
	if code := Run(nil, &stdout, &stderr, root); code != 1 || stderr.String() != "rm: govna/ac1-govna-rm-v0.31.0.md has been edited since last emission — delete or rename the emitted file before re-running\n" {
...
func TestRemovalClassificationAndTraversal(t *testing.T) {
	...
	a, err := classify(root, files, map[string]bool{"build.sh": true}, "govna/ac1-govna-rm-v0.31.0.md")
	if err != nil {
		t.Fatal(err)
	}
...
func TestRemovalGolden(t *testing.T) {
	...
	got := buildAC("govna/ac7-govna-rm-v0.31.0.md", "v0.31.0", canon.Code, "Go", a)
	...
	wrapped := emission.GuardedBody(emission.RemovalMarkerPrefix, "v0.31.0", []byte(got))
...
func TestRemovalFlavorOverride(t *testing.T) {
	...
	data, _ := os.ReadFile(filepath.Join(root, "govna", "ac1-govna-rm-v0.31.0.md"))
