...
func TestRemovalFreshAndIdempotent(t *testing.T) {
	...
	if stdout.String() != "Wrote govna/ac1-govna-rm-v0.47.0.md for review.\n" {
		t.Fatalf("stdout=%q", stdout.String())
	}
	path := filepath.Join(root, "govna", "ac1-govna-rm-v0.47.0.md")
	...
	markerPrefix := "<!-- govna-rm: emitted-by govna executable v9.8.7 with embedded canon v0.47.0 sha256:"
	...
	matches, err := filepath.Glob(filepath.Join(root, "govna", "ac*-govna-rm-v0.47.0.md"))
	...
	stdout.Reset()
	stderr.Reset()
	if code := Run(nil, &stdout, &stderr, root, testProgramVersion); code != 1 || stderr.String() != "rm: govna/ac1-govna-rm-v0.47.0.md has been edited since last emission — delete or rename the emitted file before re-running\n" {
...
func TestRemovalClassificationAndTraversal(t *testing.T) {
	...
	a, err := classify(root, files, map[string]bool{"build.sh": true}, "govna/ac1-govna-rm-v0.47.0.md")
	if err != nil {
		t.Fatal(err)
	}
...
func TestRemovalGolden(t *testing.T) {
	...
	got := buildAC("govna/ac7-govna-rm-v0.47.0.md", testProgramVersion, canon.Version, canon.Code, "Go", a)
...
func TestRemovalInstructionBranches(t *testing.T) {
	...
	body := buildAC("govna/ac7-govna-rm-v0.47.0.md", testProgramVersion, canon.Version, canon.Code, "Go", withRouting)
	ordered := []string{
	...
	for _, want := range []string{
		"Govna executable v9.8.7 created this removal plan from its embedded governance files (canon v0.47.0).",
...
func TestRemovalFlavorOverride(t *testing.T) {
	...
	data, _ := os.ReadFile(filepath.Join(root, "govna", "ac1-govna-rm-v0.47.0.md"))
