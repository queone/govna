...
The AC is the implementation contract for one approved roadmap item. An AC records settled intent, scope, and proof once so later review, editing, and verification can reuse the same context instead of reconstructing it. The full editing cycle lives in `govna/editing-cycle.md`; `AGENTS.md` governs drafting, review, and integration.
