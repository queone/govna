...
5. **Implement the settled scope.** Implement content, structure, and direct doc updates together; run validation, adversarial verification, defect correction, and the closure audit. Pause for Ratify.
...
7. **Perform Package only when explicitly requested.** Accept standalone `Package`, `package`, `pack`, or `prep` only after Ratify acceptance; follow `govna/release.md` for the existing checklist and implementation.
