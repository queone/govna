Copy this file to `govna/ac<N>-<slug>.md`; use a kebab-case slug and `# AC<N> Title` heading.

Set `N` to one above the highest AC number in `govna/` or `git log --all --pretty=%B`; count every reference because release-prep deletions do not reset numbering.
...
List concrete changes and exact paths under useful groupings. Treat this list as authoritative; apply only the effective-scope and emitted-routing exceptions defined in `AGENTS.md`.
...
Label every AT `[Automated]` or `[Manual]` and `[Pre-release gate]` or `[Post-release verification]`. Prefer automated pre-release coverage; use post-release only when automated regression coverage already gates the behavior class.
