...
6. **Ratify the delivered AC.** Treat standalone Ratify as acceptance, complete it in the same turn after a clean review, and request no second acceptance signal. Return contract or scope feedback to Refine without completing Ratify. Auto-correct an implementation-only finding inline, rerun applicable validation — skipping `./build.sh` only for a documentation-only correction not covered by this repo's own build validation — and repeat the review automatically for at least 3 rounds before returning a still-unresolved finding to Implement without completing Ratify.
