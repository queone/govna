...
test_govna_prep_version_mutation() {
  local fixture before output rc
  fixture=$(new_fixture) || fail 'create Govna version fixture'
  mkdir -p "$fixture/src"
  printf 'const PROGRAM_VERSION: &str = "1.2.3";\n' >"$fixture/src/main.rs"
  (
    cd "$fixture" || exit 1
    _replace_govna_program_version 1.3.0
    assert_contains "$(cat src/main.rs)" '"1.3.0"'
  ) || fail 'replace Govna PROGRAM_VERSION'
  printf 'const PROGRAM_VERSION: &str = "bad";\n' >"$fixture/src/main.rs"
  before=$(cat "$fixture/src/main.rs")
  set +e
  output=$(cd "$fixture" && _govna_program_version_info 2>&1)
  rc=$?
  set -e
  assert_equal "$rc" 1
  assert_equal "$(cat "$fixture/src/main.rs")" "$before"
  assert_contains "$output" 'malformed PROGRAM_VERSION'
  rm -rf -- "$fixture"
  pass
}
